...
@section('content')
    ...
    <section class="jumbotron text-center">
        <div class="container">
          <h1 class="jumbotron-heading">{{ $news->title }}</h1>
          <p class="lead text-muted">{!! $news->synthesis !!}</p>
        </div>
      </section>
    ...
    <div class="container">
        <div class="row mt-5 mb-5">
            ...
            <div class="col-lg-12">
                <small style="font-size: 12px">SECCION: {{ $news->section->name }}</small>
            </div>
        </div>
        <!-- NOTICIA HEADER -->
        <div class="row">
        ...
        <div class="row">
            <div class="col-md-12">
                {{-- Archivos adjuntos --}}
                <hr>
                @if($mainFile = $news->files->where('main_file', 1)->first())
