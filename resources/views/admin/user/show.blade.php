        ...
        <div class="col-xs-12 col-md-3 col-lg-2 profile-left">
            <div class="profile-left-heading">
                ...
                <a href="" class="profile-photo"><img class="img-circle img-responsive"
                                                      src="https://ui-avatars.com/api/?name={{ str_replace(' ', '+', ucwords($profile->name)) }}"
                                                      alt=""></a>
                ...
                <ul class="list-group">
                    @foreach($countNews as $count)
                        <li class="list-group-item">{{ $count['label'] }} <a
                                    href="javascript:void(0)">{{ number_decimal($count['value']) }}</a></li>
                    @endforeach
                </ul>


                <a href="{{ route('admin.user.edit', ['id' => $profile->id ]) }}"
                   class="btn btn-danger btn-quirk btn-block profile-btn-follow">Editar</a>
            </div>
            <div class="profile-left-body">
                ...
                <p>
                    <i class="glyphicon glyphicon-user mr5"></i> {{ $profile->getMetaByKey('user_address') ? $profile->getMetaByKey('user_address')->meta_value : "-" }}
                </p>
                ...
                <p>
                    <i class="glyphicon glyphicon-phone mr5"></i>Tel: {{ $profile->getMetaByKey('user_phone') ? $profile->getMetaByKey('user_phone')->meta_value : "-" }}
                </p>
                <p>
                    <i class="glyphicon glyphicon-phone mr5"></i>Whatsapp: {{ $profile->getMetaByKey('user_whatsapp') ? $profile->getMetaByKey('user_whatsapp')->meta_value : "-" }}
                </p>
                ...
                <ul class="list-inline profile-social">
                    @if($profile->getMetaByKey('user_facebook'))
                        <li><a href="{{ $profile->getMetaByKey('user_facebook')->meta_value }}"><i
                                        class="fa fa-facebook-official"></i></a></li>
                    @endif
                    @if($profile->getMetaByKey('user_twitter'))
                        <li><a href="{{ $profile->getMetaByKey('user_twitter')->meta_value}}"><i
                                        class="fa fa-twitter"></i></a></li>
                    @endif
                    @if($profile->getMetaByKey('user_instagram'))
                        <li><a href="{{ $profile->getMetaByKey('user_instagram')->meta_value}}"><i
                                        class="fa fa-instagram"></i></a></li>
                    @endif
                    @if($profile->getMetaByKey('user_linkedin'))
                        <li><a href="{{ $profile->getMetaByKey('user_linkedin')->meta_value}}"><i
        ...
        <div class="col-md-9 col-lg-10 profile-right">
            <div class="profile-right-body">
                ...
                <ul class="nav nav-tabs nav-justified nav-line">
                    <li class="active"><a href="#activity" data-toggle="tab"><strong>{{ __('Ultimas notas') }}</strong></a>
                    </li>
                    @if ($profile->isMonitor())
                        <li><a href="#send-news" data-toggle="tab"><strong>Noticias Enviadas</strong></a></li>
                    @endif
                    @if ($profile->isExecutive())
                        <li><a href="#companies" data-toggle="tab"><strong>Empresas ({{ $profile->companies->count() }}
                                    )</strong></a></li>
                        <li><a href="#themes" data-toggle="tab"><strong>Temas</strong></a></li>
                    @endif
                    @if ($profile->isAdmin())
                        <li><a href="#companies" data-toggle="tab"><strong>Empresas({{ App\Models\Company::count() }}
                                    )</strong></a></li>
                        <li><a href="#themes" data-toggle="tab"><strong>Temas</strong></a></li>
                    @endif
                            ...
                            <div class="row">
                                <div class="col-md-3 col-md-offset-9">
                                    <button data-href="{{ route('admin.executive.add.company') }}"
                                            ...
                                            id="btn-assign-company">Asignar empresa
                                    </button>
                                </div>
                            </div>
                                        ...
                                        <li class="nav-parent active">
                                            ...
                                            <ul class="children">
                                                ...
                                                @foreach ($theme->assignedNews as $noteAssigned)
                                                    ...
                                                    @endif
                                                    <li>
                                                        <a href="{{ route('admin.new.show', ['id' => $noteAssigned->news->id]) }}">{{ $noteAssigned->news->title }}</a>
                                                    </li>
                                                    @php
                                                        $countNote++;
                                                    @endphp
