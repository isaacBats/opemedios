                    ...
                    <form action="{{ route('users') }}" method="GET">
                        <div class="row">
                            <div class="col-md-2 form-group">
                                ...
                                <input type="text" name="name" class="form-control" id="input-user-name"
                                       value="{{ request()->get('name') }}">
                            </div>
                            <div class="col-md-2 form-group">
                                <label for="input-user-email" class="text-muted">Correo</label>
                                <input type="text" name="email" class="form-control" id="input-user-email"
                                ...
                                <select class="form-control" name="roll">
                                    ...
                                    @foreach(Spatie\Permission\Models\Role::all() as $role)
                                        @if($role->name === 'disable')
                                            @continue
                                        @endif
                                        <option value="{{ $role->id }}" {{ request()->get('roll') == $role->id ? 'selected' : '' }}>{{ App\Models\User::getRoleNameCustom($role->name) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-2 form-group">
                            ...
                            <div class="col-md-4 form-group" style="margin-top: 20px">
                                ...
                                @if(request()->has('name') || request()->has('email') || request()->has('roll'))
                                    <a href="{{ route('users') }}" class="btn btn-warning ml-2"
            ...
            <div class="panel-heading">
                <div class="row">
                    ...
                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 text-right">
                        <a href="{{ route('register.user') }}" class="btn btn-danger btn-quirk btn-lg"><i
                        ...
                        @foreach($users as $user)
                            <tr>
                                ...
                                <td class="text-left">
                                    <a href="{{ route('user.show', ['id' => $user->id]) }}">
                                        <div class="td-select-all">
                                            {{ App\Models\User::getRoleNameCustom($user->roles->first()->name) }}
                                        </div>
                                    </a>
                                </td>
                                <td class="text-left">
