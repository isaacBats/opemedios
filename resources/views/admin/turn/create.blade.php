    ...
    <div class="col-sm-12 col-md-12">
        <div class="panel">
            <div class="panel-heading">
                <h4 class="panel-title">{{ __('Nuevo Giro') }}</h4>
                <hr>
            </div>
            <div class="panel-body">
                <form action="{{ route('admin.turns.store') }}" method="POST" class="form-horizontal">
                    @csrf
                    <div class="form-group">
                        <label class="col-sm-3 control-label">{{ __('Nombre') }}<span class="text-danger">*</span></label>
                        <div class="col-sm-8">
                            <input type="text" name="name" class="form-control" placeholder="Nombre del giro" value="{{ old('name') }}" required />
                        </div>
                        @error('name')
