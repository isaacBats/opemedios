...
<body>
    ...
    <table 
        style="background-image: url(https://i.ibb.co/G2cVw0w/fondo.jpg); padding:50px 20px;max-width:600px;font-size:14px;margin:auto;height:auto;width:100%; font-family: Arial;">
        <thead style="display:block">
            ...
            <tr style="float:right">
                <td>
                    <img src="{{ asset("images/{$newsletterSend->newsletter->banner}") }}" alt="{{ $newsletterSend->newsletter->name }}" style="max-width:250px;float:right">
                    @php
            ...
            @foreach ($newsletterSend->newsletter->company->themes as $theme)
                @if($newsletterSend->newsletter_theme_news->where('newsletter_theme_id', $theme->id)->count())
                    <tr>
                        <td style="margin:auto auto 20px;width:max-content;text-align:left;display:block">
                            <p style="margin:auto auto 10px;background:#023f82;text-align:center;font-size:24px;color:white;display:flex;padding:10px;width:260px">{{ strtoupper($theme->name) }} <span style="margin: auto 5px auto auto;"><img src="https://i.ibb.co/3CKhN7H/triangulo-blanco.png" alt="triangulo-blanco"
                                style="width: 20px;"></span></p>
                            @php
                ...
                @foreach ($newsletterSend->newsletter_theme_news as $note)
                    @if($note->theme->id == $theme->id)
                        <tr style="margin:auto auto 60px;background:white;display:block;padding:20px;width:80%">
                            <td>
